...
You'll need [Python 3.10](https://www.python.org/downloads/) or later and pytest
...
To run the validator from a Linux shell or in a macOS terminal, run this command:
...
* To add a new schema with an existing namespace, drop the schema file under the existing namespace directory, e.g. `schema/<schema_namespace>`. 
* To add a new namespace, create a new namespace directory under the `schema/` base directory. You'll also need to configure the corresponding base URI prefix, as discussed below. 
...
If a test data file is loaded by the validator but no corresponding local or remote schema is found, the test will fail:
...
To create a new namespace, add a dictionary entry to `schema_namespace_uris` in `conftest.py`.
